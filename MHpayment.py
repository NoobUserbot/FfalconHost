...
def _create_yoomoney_invoice(recipient: int, amount: float, comment: str) -> str:
    ...
    paym = YM.Quickpay('4100118021186932', 'shop', 'Buy subscription for falconhost', 'SB', amount, label=comment)
...
def _check_yoomoney_payment(recipient: int) -> bool:
    try:
        ...
        history = client.operation_history(label=f'falcon.host subscription for {recipient}')
    ...
    if response.ok:
        ...
        try:
            for payment in response.json().get('data', []):
                if payment.get('comment', '') == f'falcon.host subscription for {recipient}' and payment.get('txnId', '') not in op_cdb['qvpast']:
...
def _check_crypto_payment(recipient: int, amount: int = 50) -> bool:
    ...
    for r in results:
        if r['payload'] == f'falcon.host subscription for {recipient}':
            if r['status'] == 'paid':
                op_cdb = load(open('bezahl.json', 'r', encoding='utf-8'))
